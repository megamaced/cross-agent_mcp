...
`session_id`에 대화 이름을 그대로 넣어도 된다. **부분 일치는 하지 않는다** — Claude 세션에는
고유한 이름이 없고 제목이 곧 사람이 처음 친 말이라, 그 안에 인용된 경로 하나가 이름 행세를
한다. 실제로 `koppa_studio`가 몇 달 전 세션의 첫 메시지에 적힌 경로와 맞아떨어져, 아무도
보고 있지 않은 세션을 헤드리스로 되살린 사고가 있었다.
