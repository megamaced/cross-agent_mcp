...
**이름으로 지정할 수 있다.** 사람은 uuid가 아니라 이름으로 대화를 부르므로,
`session_id`에 대화 이름을 그대로 넣어도 된다(정확 일치 우선, 없으면 부분 일치, 그중 최신).
...
- **배달 큐는 서버 프로세스 안에만 있다** — 아직 디스크에 영속화하지 않는다. 배달이
  진행 중인데 발신자 세션이 종료되면 그 MCP 서버 프로세스와 함께 워커도 사라지고 답변은
  유실된다(상대 트랜스크립트에는 남는다). 대화형 세션에서는 문제되지 않지만, 헤드리스
  `claude -p` 처럼 턴이 끝나면 바로 종료되는 호출에서 보낸 메시지는 답을 되받지 못한다.
- **다른 VS Code 창의 세션** — 패널 탐색이 프로세스 조상 체인으로 "이 창"을 판별하므로,
  다른 창에서 열린 대화는 패널 경로로 갈 수 없고 헤드리스 CLI resume으로 폴백한다.
  그 세션을 다른 창의 패널이 살아 있는 채로 물고 있으면 CLI가 thread-store writer 충돌로
  거부한다. 이 경우 대상 대화를 같은 창에서 열어야 한다.
